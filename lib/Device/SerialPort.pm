...
@LOCATIONS = (
    '/usr/local/lib/site_perl/i386-linux/termios.ph',        # Linux
    '/usr/local/lib/site_perl/i386-linux/asm/termios.ph',    # Linux
    'sys/termiox.ph',                                        # AIX
...
if ($SerialJunk::ioctl_ok) {
    eval {
        ...
        $txdone   =
          defined(&SerialJunk::TIOCSERGETLSR) ? &SerialJunk::TIOCSERGETLSR : 0;
        ...
        $termioxflow =
          defined(&SerialJunk::CTSXON)
          ? ( &SerialJunk::CTSXON | &SerialJunk::RTSXOFF )
          : 0;
...
sub TIOCM_LE {
    if ( defined &SerialJunk::TIOCSER_TEMT ) {
        return &SerialJunk::TIOCSER_TEMT;
    }
...
%EXPORT_TAGS = (
    ...
    PARAM => [
        qw( LONGsize	SHORTsize	OS_Error
          nocarp		yes_true )
    ]
);
...
my @termios_fields =
  ( "C_CFLAG", "C_IFLAG", "C_ISPEED", "C_LFLAG", "C_OFLAG", "C_OSPEED" );
...
sub new {
    ...
    my $nameOrConf = shift;
    return start( $class, $nameOrConf, @_ )
      if ( -f $nameOrConf && !-c $nameOrConf );
    ...
    if ($lockfile) {
        ...
        my $lockf = POSIX::open( $self->{LOCK},
            &POSIX::O_WRONLY | &POSIX::O_CREAT | &POSIX::O_NOCTTY |
              &POSIX::O_EXCL );
    ...
    $self->{FD} =
      POSIX::open( $self->{NAME},
        &POSIX::O_RDWR | &POSIX::O_NOCTTY | &POSIX::O_NONBLOCK );
...
sub can_rts {
    return 0
      unless ( $bitset && $bitclear && $rtsout && !( $dtrset && $dtrclear ) );
    return 1;
}
...
sub read_vmin {
    ...
    my $tout;
    my $ready =
      select( $rout = $rin, $wout = undef, $eout = $ein, $tout = $tin );
