...
sub add_global_handler {
    ...
    return $self->_add_generic_handler( $event, $ref, $rp, \%_udef,
        'add_global_handler' );
}
...
sub add_handler {
    my ( $self, $event, $ref, $rp ) = @_;
    return $self->_add_generic_handler( $event, $ref, $rp, $self->{_handler},
        'add_handler' );
}
...
sub connect {
    ...
    unless ( $self->hostname ) {
        ...
        if ($host) {
            ...
            croak
"Can't determine this machine's hostname! Please specify your hostname with the LocalAddr parameter to connect().";
        }
    }
    unless ( $self->nick ) {
        $self->nick( $ENV{IRCNICK}
              || eval { scalar getpwuid($>) } || $ENV{USER}
              || $ENV{LOGNAME}
              || "WankerBot" );
    }
    unless ( $self->port ) {
    ...
    unless ( $self->ircname ) {
        $self->ircname( $ENV{IRCNAME}
              || eval { ( getpwuid($>) )[6] } || "Just Another Perl Hacker" );
    }
    ...
    if (
        connect(
            $sock, sockaddr_in( $self->port, inet_aton( $self->server ) )
        )
      )
    {
        $self->socket($sock);
        ...
        carp( sprintf "Can't connect to %s:%s!", $self->server, $self->port );
        $self->error(1);
        return;
    }
...
sub ctcp {
    ...
    if ( $type eq "PING" ) {
        ...
        unless (
            $self->sl(
                "PRIVMSG $target :\001$type " . CORE::join( " ", @_ ) . "\001"
            )
          )
        {
            carp "Socket error sending $type request in ctcp()";
        ...
        unless (
            $self->sl(
                "PRIVMSG $target :\001ERRMSG " . CORE::join( " ", @_ ) . "\001"
            )
          )
        {
            carp "Socket error sending $type request in ctcp()";
...
sub disconnect {
    ...
    $self->socket(undef);
    $self->handler(
        Net::IRC::Event->new( "disconnect", $self->server, '', @_ ) );
}
...
sub format {
    ...
    if (@_) {
        ...
        return ( $self->{'_format'}->{$ev} || $self->{'_format'}->{'default'} );
    }
}
...
sub links {
    ...
    $self->sl(
        "LINKS" . ( scalar(@_) ? " " . CORE::join( " ", @_[ 0, 1 ] ) : "" ) );
}
...
sub new_get {
    ...
    unless (defined $handle
        and ref $handle
        and ( ref $handle eq "GLOB" or $handle->can('print') ) )
    {
    ...
    my $dcc =
      Net::IRC::DCC::GET->new( $self, $nick, $address, $port, $size, $name,
        $handle, $offset );
...
sub parse {
    my ($self) = shift;
    my (
        $from, $type,  $message, @stuff,    $itype,
        $ev,   @lines, $line,    $lineplus, $dummy
    );
    ...
    if ( defined recv( $self->socket, $line, 10240, 0 )
        and ( length( $self->{_frag} ) + length($line) ) > 0 )
    {
  ...
  PARSELOOP: foreach $line (@lines) {
        ...
        if ( $line =~ /^PING/ ) {
            $ev = (
                Net::IRC::Event->new(
                    "ping", $self->server, $self->nick, "serverping",   # FIXME?
                    substr( $line, 5 )
                )
            );
            ...
            $ev =
              Net::IRC::Event->new( "snotice", $self->server, '', 'server',
                ( split /:/, $line, 2 )[1] );
          ...
          )    # That ought to do it for now...
        {
                ...
                or $type eq "invite" )
            {
                ...
                $ev =
                  Net::IRC::Event->new( $type, $from, shift(@stuff), $type,
                    @stuff, );
            }
            elsif ( $type eq "quit" or $type eq "nick" ) {

                $ev =
                  Net::IRC::Event->new( $type, $from, $from, $type, @stuff, );
            }
            elsif ( $type eq "kick" ) {

                $ev =
                  Net::IRC::Event->new( $type, $from, $stuff[1], $type,
                    @stuff[ 0, 2 .. $#stuff ],
                  );
                ...
                $ev =
                  Net::IRC::Event->new( $type, $from, '', $type, $line )
                  ;    # Ahh, what the hell.
            }
            elsif ( $type eq "wallops" ) {
                $ev = Net::IRC::Event->new( $type, $from, '', $type, $line );
            }
            else {
          ...
          )
        {
            ...
            $ev =
              Net::IRC::Event->new( 'umode', $self->server, $self->nick,
                'server', substr( $line, index( $line, ':', 1 ) + 1 ) );
          ...
          )
        {
            ...
            if ( $line =~ /^ERROR :Closing [Ll]ink/ ) {    # is this compatible?
                ...
                $ev =
                  Net::IRC::Event->new( "error", $self->server, '', 'error',
                    ( split /:/, $line, 2 )[1] );
            }
        }
...
sub parse_ctcp {
    ...
    while ( ( $one, $two ) = ( splice @foo, 0, 2 ) ) {
        ...
        elsif ( $type eq 'public' or $type eq 'msg' ) {
            $prefix = 'c';
        }
        else {
        ...
        if ($prefix) {
            ...
            $self->handler(
                Net::IRC::Event->new( $handler, $from, $stuff, $handler, $one )
            );
        }

        $self->handler(
            Net::IRC::Event->new( $type, $from, $stuff, $type, $two ) )
          if $two;
    }
    return 1;
}
...
sub parse_num {
    ...
    return Net::IRC::Event->new( $type, $from, '', 'server', @stuff );
}
