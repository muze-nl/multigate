...
sub fixaddr {
    ...
    if ( $address =~ /^\d+$/ ) {
        ...
        $address =~ /^[12]?\d{1,2}\.[12]?\d{1,2}\.[12]?\d{1,2}\.[12]?\d{1,2}$/ )
    {
        return $address;
    }
    ...
    if ( defined $sock->recv( $input, 10240 ) ) {
        ...
        if ( length($frag) > 0 ) {
            ...
            $self->{_parent}->handler(
                Net::IRC::Event->new(
                    'dcc_close',      $self->{_nick},
                    $self->{_socket}, $self->{_type}
                )
            );
...
sub new {
    ...
    my (
        $class, $container, $nick,   $address, $port,
        $size,  $filename,  $handle, $offset
      )
      = @_;
    my ( $sock, $fh );
    ...
    if ( defined $sock ) {
        $container->handler(
            Net::IRC::Event->new(
                'dcc_open', $nick, $sock, 'get', 'get', $sock
            )
        );
...
sub parse {
    ...
    $self->{_parent}->handler(
        Net::IRC::Event->new(
            'dcc_update', $self->{_nick}, $self, $self->{_type}, $self
        )
    );
}
...
sub close {
    ...
    $self->{_parent}->handler(
        Net::IRC::Event->new(
            'dcc_close', $self->{_nick}, $self->{_socket}, $self->{_type}
        )
    );
    $self->{_socket}->close;
    return;
...
sub new {
    ...
    if ($type) {
        ...
        if ( defined $sock ) {
            $container->handler(
                Net::IRC::Event->new(
                    'dcc_open', $nick, $sock, 'chat', 'chat', $sock
                )
            );
        }
        else {
        ...
        $self->{_parent}
          ->parent->addfh( $self->socket, $self->can('parse'), 'r', $self );
    }
...
sub parse {
    ...
    foreach my $line ( $self->_getline($sock) ) {
        ...
        $self->{_parent}->handler(
            Net::IRC::Event->new(
                'chat', $self->{_nick}, $self->{_socket}, 'chat', $line
            )
        );
...
sub parse {
    ...
    $self->{_parent}->{_parent}->handler(
        Net::IRC::Event->new(
            'dcc_open',                  $self->{_parent}->{_nick},
            $self->{_parent}->{_socket}, $self->{_parent}->{_type},
            $self->{_parent}->{_type},   $self->{_parent}->{_socket}
        )
    );
}
