...
my $strVERSION = "MSN 2.1.2 (14/05/2006) $REVISION";
...
sub consolemsg
{
	...
	if( defined $self->{handler}->{Messaging} )
	{
		$self->call_event( $self, 'Messaging', $message );
	}
	elsif( $self->{Messaging} )
	{
		#print STDERR "$message\n";
	}
...
sub serverError
{
	...
	{
		#print STDERR "SERVER ERROR : $message\n";
	}
...
sub error
{
	...
	{
		$self->call_event( $self, 'Error', $self->{LastError} );
	}
	elsif( $self->{Error} )
	{
		...
		for( my $i=0; $i<20; $i++ )
		{
			...
			#print STDERR "  $i: $subroutine ($filename, line $line)\n";
		}
	}
...
sub cmdError
{
	...
	#print STDERR "UNDEFINED CMD : $message\n" if( $self->{CMDError} );
...
Expects $list to be one of PL, FL, RL, AL, or BL.  Returns the email addresses on said list.
...
sub do_one_loop
{
	...
	foreach my $fh ( @ready )
	{
		...
		while( $connection->{buf} =~ s/^(.*?\n)// )
		{
			...
			#print STDERR "($fn $connection->{Type}) RX: $incomingdata\n" if( $self->{ShowRX} );
