...
use DBI;
use Date::Calc
  qw( Language Decode_Language Delta_DHMS Add_Delta_YMD Today Today_and_Now );
...
EOT
    ...
    my ( $Dd, $Dh, $Dm, $Ds ) =
      Delta_DHMS( $ty, $bm, $bd, $bh, $bmin, $bs, $ty, $tm, $td, $th, $tmin,
        $ts );
    if (   ( $Dd < 0 )
        || ( ( $Dd == 0 ) && ( $Dh < 0 ) )
        || ( ( $Dd == 0 ) && ( $Dh == 0 ) && ( $Dm < 0 ) )
        || ( ( $Dd == 0 ) && ( $Dh == 0 ) && ( $Dm == 0 ) && ( $Ds < 0 ) ) )
    {
        ...
        ( $Dd, $Dh, $Dm, $Ds ) = Delta_DHMS(
            $ty - 1, $bm, $bd, $bh, $bmin, $bs,
            $ty,     $tm, $td, $th, $tmin, $ts
        );
    }

    print
"$user system boot $bdt $btm, uptime $Dy years, $Dd days, $Dh hours, $Dm minutes, $Ds seconds.\n";
