...
if ( lc($command) eq "list" ) {
    if ($rest) {
        ...
        unless ( ( $userlevel >= $adminlevel )
            or ( get_group_admin_flag( $rest, $realuser ) eq "yes" ) )
        {
    ...
    unless ( ( $userlevel >= $adminlevel )
        or ( get_group_admin_flag( $group, $realuser ) eq "yes" ) )
    {
    ...
    if ( user_exists($user) ) {
        if ( add_group_member( $group, $user ) ) {
            ...
            if ( defined $protocol ) {
                ...
                {
                    print
"Setting group-protocol for $group,$user to $protocol, \"$bla\"\n";
                }
                else {
                    print
"Failed setting group-protocol for $group,$user to $protocol\n";
                }
            }
        }
        else {
