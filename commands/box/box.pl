...
if (    ( $userlevel < $adminlevel )
    and ( ( lc($command) ne 'get' ) or ( $user ne $realuser ) ) )
{
    print "Not allowed\n";
    exit 0;
}
...
if ( lc($command) eq "add" ) {
    ...
    if ( defined $amount ) {
        ...
        if ($res) {
            ...
            print
"Added $amount to '$box' for '$user'. (New balance: $new_amount)\n";
        }
        else {
            print
"A problem occured when adding $amount to '$box' for user '$user'.\n";
        }
    }
    else {
    ...
    if ( defined $amount ) {
        ...
        if ($res) {
            my $new_amount = get_box( $box, $user );
            print
"Deleted $amount from '$box' for '$user'. (New balance: $new_amount)\n";
        }
        else {
            print
"A problem occured when deleting $amount from '$box' for user '$user'.\n";
        }
    }
    else {
        print "Usage: box inc $box $user <amount>\n";
    ...
    if ( defined $amount ) {
        ...
        if ($res) {
            ...
            print
"A problem occured when setting '$box' for user '$user' to $amount.\n";
        }
    }
    else {
    ...
    if ( defined $cur_amount ) {
        ...
        print
          "Problem getting balance of box '$box' for '$user'. Does it exist?\n";
    }
}
else {
